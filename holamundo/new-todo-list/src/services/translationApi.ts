...
export class Traductor {
	...
	static async fetchTraduccionesAsync() {
		Traductor.traducciones = await this.getTraduccionesAsync();
	}

	static async getTraduccionesAsync(): Promise<object> {
		return new Promise((resolve) => {
			setTimeout(() => {
				resolve(Traductor.getTraducciones());
			}, 1000);
		});
	}

	static async getTraduccionesPaginaAsync(idioma: string, page: string): Promise<object> {
		return new Promise((resolve) => {
			setTimeout(() => {
				resolve(Traductor.getTraduccionesPagina(idioma, page));
			}, 1000);
		});
	}

	static getTraduccionesPagina(idioma: string, page: string): object {
	...
	public traducir(page: string, label: string, defecto: null | string = null): string {
		const paginaObj = (this.traducciones as any)[this.idioma]?.[page] as Record<string, string> | undefined;
		...
		if (!paginaObj || !(label in paginaObj)) {
			...
			Traductor.insertTraduccion(page, label, defecto ?? '');
			return defecto ?? '';
		}
	...
	static insertTraduccion(page: string, label: string, traduccion: string) {
		// Llamada a Api
		console.log(`Insertando traducción: ${label} -> ${traduccion} en ${page}`);
	}
